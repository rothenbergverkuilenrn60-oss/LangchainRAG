...
用户：POST /query {"question": "高血压用什么药？", "tenant_id": "default"}
    ...
    ⚠️ 注意：降压药物的选择需根据患者合并症（如糖尿病、慢性肾病）
...
| 阶段 | 操作 | 典型耗时 |
|------|------|---------|
| 查询扩展 | 3个并发 LLM 调用 | 1-2s |
| 混合检索 | 6变体 × 双路并发 | 0.3-0.8s |
| 双层 RRF + 去重 | 纯 CPU 计算 | < 10ms |
| Reranker | CrossEncoder 推理 60对 | 0.5-1s（GPU） |
| 上下文处理 | 压缩+重排+格式化 | < 5ms |
| 首 token 延迟 | LLM 开始输出 | 0.5-1s |
| 完整回答生成 | 约 500 字的回答 | 8-15s（流式推送） |
| **用户感知延迟** | **首字出现** | **< 4s** |

### 为什么这个架构达到500强水准？

1. **双层混合检索（BM25+向量）**：单纯向量检索对关键词匹配弱，单纯 BM25 对语义理解弱，双路互补
2. **双层 RRF**：Layer1 解决量纲问题（BM25分数 vs 余弦相似度），Layer2 解决多查询聚合
3. **Parent-Child 索引**：子块精确命中 + 父块完整上下文，解决"检索精确 vs 上下文充分"的矛盾
4. **HyDE**：让 LLM 生成"理想答案"，再用它检索，绕过用户问题的模糊性
5. **BGE-Reranker 精排**：CrossEncoder 是当前精度最高的检索重排方案（超过 GPT-4 embedding）
6. **Self-RAG**：自动验证，减少幻觉，提升医学场景的安全性
7. **流式输出**：首 token < 4s，用户体验媲美 ChatGPT
8. **多租户**：生产级数据隔离，支持多机构部署
9. **实体识别 + 元数据过滤**：支持精确过滤（如只检索"心血管章节"），大幅提升精度
10. **规则引擎**：医学领域知识注入，让 RAG 系统真正"懂"医学
